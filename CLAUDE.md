...
- **Не сделано (вернуться позже):** включение/выключение по расписанию. Делать в Go-сервисе
  рядом с `BRIGHTNESS_SCHEDULE` (тот же паттерн внутрисервисного планировщика), а не cron'ом.
