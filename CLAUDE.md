...
### Боковая RGB-подсветка (ambient light) — вкл/выкл по LAN (найдено 2026-08-09)

У корпуса Times Frame сбоку есть вертикальная цветная светодиодная линия. **В приложении
Divoom настройки для неё нет** (пользователь проверил) — управлять можно только по LAN API.
В документации Divoom команд тоже нет, но **прошивка Frame их знает** (найдено перебором,
проверено вживую):
...
curl -s -X POST http://192.168.178.40:9000/divoom_api \
  ...
  -d '{"Command":"Channel/GetAmbientLight","ReturnCode":0}'
# → {"Brightness":100,"ColorCycle":0,"EqOnOff":0,"Color":"#ffffff","SelectEffect":1, …}

# ВЫКЛЮЧИТЬ
curl -s -X POST http://192.168.178.40:9000/divoom_api \
  -H 'Content-Type: application/json; charset=UTF-8' \
  -d '{"Command":"Channel/SetAmbientLight","ReturnCode":0,"Brightness":0,"ColorCycle":0,"EqOnOff":0,"Color":"#000000","SelectEffect":0}'

# ВКЛЮЧИТЬ — текущий пресет пользователя: белый, вся линия плавно разгорается и гаснет
curl -s -X POST http://192.168.178.40:9000/divoom_api \
  -H 'Content-Type: application/json; charset=UTF-8' \
  -d '{"Command":"Channel/SetAmbientLight","ReturnCode":0,"Brightness":100,"ColorCycle":0,"EqOnOff":0,"Color":"#ffffff","SelectEffect":6}'
```

- Поля: `Brightness` 0–100 (0 = панель погашена), `Color` `#rrggbb` (статичный цвет),
  `ColorCycle` 1 = перелив по спектру (тогда `Color` — лишь базовый; работает в паре с любым
  `SelectEffect`), `EqOnOff` 1 = **реакция на звук** — панель дёргается в такт (проверено),
  `SelectEffect` — режим свечения.
- **`SelectEffect` — все значения выверены визуально (2026-08-09), валидный диапазон `0..7`:**

  | id | что делает |
  |----|------------|
  | `0` | горит **только нижняя часть** линии, статично (при любой `Brightness`, в т.ч. 100) |
  | `1` | вся линия целиком, статично |
  | `2` | волна: свет плавно бежит снизу вверх |
  | `3` | плавное мерцание («дыхание») |
  | `4` | мерцание, заметно активнее чем `3` |
  | `5` | маленький бегунок сверху вниз |
  | `6` | вся линия плавно разгорается и плавно гаснет — **текущий выбор пользователя** |
  | `7` | вся линия статично, без анимаций (визуально дубль `1`) |

  ⚠️ Значение **вне диапазона молча схлопывается в `0`**: на `SelectEffect:8` устройство
  записало `0` (видно в `GetAmbientLight`) — то есть панель уйдёт в «горит только низ».
  Всегда перечитывать состояние после записи, если значение вычисляемое.
- Записанное состояние **сохраняется** и читается обратно через `GetAmbientLight`.
- ⚠️ **`Channel/SetAmbientLight` без полей обнуляет всю структуру** (так исходные заводские
  настройки подсветки и были затёрты при разведке). Всегда слать полный набор полей.
- **Как искать другие недокументированные команды Frame:** отправить `{"Command":"Ns/Action",
  "ReturnCode":0}` на `/divoom_api` — неизвестная команда отвечает `ReturnCode 1 /
  "Only accept JSON parameters"`, известная — `ReturnCode 0`. Осторожно: у распознанной
  Set-команды побочный эффект применится сразу (см. предыдущий пункт), так что перебирать
  безопасно только `Get*`.
- ⚠️ Исходные заводские значения подсветки **утрачены** (затёрты пустым Set при разведке) и
  восстановить их из приложения нельзя — там настройки нет. Текущий эталон «включено» —
  пресет выше (`SelectEffect:6`, белый, 100 %).
- **Расписание — СДЕЛАНО (2026-08-10):** подсветкой рулит сам Go-сервис (`internal/app/ambient.go`),
  тем же паттерном, что `BRIGHTNESS_SCHEDULE` (goroutine спит до точки, без cron'а). Дефолт:
  `AMBIENT_SCHEDULE=07:00=on,22:30=off` в `CLOCK_TZ`. **Каждое включение — свой случайный вид**
  (эффект из `AMBIENT_EFFECTS`, цвет из `AMBIENT_COLORS`, с шансом `AMBIENT_COLOR_CYCLE_CHANCE`
  вместо цвета — перелив по спектру), эффект прошлого дня не повторяется. Яркость — 70 %
  (`AMBIENT_BRIGHTNESS`, выбрано пользователем 2026-08-10). `EqOnOff` всегда 0.
  При старте сервис читает `GetAmbientLight` и, если состояние расходится с расписанием
  (сервис лежал в момент переключения), доводит его; совпало — не трогает (не перерисовывает вид).
  После записи всегда `GetAmbientLight` + сверка (`AmbientLight.SameAs`) — из-за схлопывания
  `SelectEffect` в `0`. Ручная проверка без ожидания точки: `clock --ambient on|off`.
...
- **Часы — нативный слой устройства** (`disp 4`, создаётся один раз через
  ...
  ⚠️ **ДВОЙНОЙ БУФЕР — обязателен (диагностировано 2026-08-28).** Прибор перерисовывает фон
  только когда `SetClockSelectId` называет **другой** циферблат, чем тот, что на экране;
  select уже выбранного id — тихий no-op (`ReturnCode 0`, но экран не меняется). Раньше он
  форс перерисовки давал, значит поведение сменилось на стороне устройства (код сервиса не
  менялся с 2026-08-16). Симптом: `SysUpdateTime` растёт каждые 30 с (фон пишется), нативные
  часы тикают, а страница висит — после ребута на той, что лежала во флеше. Лечение в
  `app.pushFrame`: два одинаковых циферблата (`DIVOOM_CLOCK_ID` + `DIVOOM_CLOCK_ID_B`,
  поле `App.dials`), кадр пишется в тот, что сейчас НЕ на экране, и он же выбирается —
  каждый select реальный, чужая тема не мелькает, команд столько же. Отсутствующий id
  создаётся при первом кадре в свой слот (`createDial`) и логируется для закрепления в ENV.
  Одноразовый `--push` перерисовки не даёт сам по себе — он отскакивает через второй
  циферблат (`cmd/clock/main.go: pushOnce`).
...
- `getSecurityInfo` (котировки, **без авторизации**): цена = `ltp`, валюта = `base_currency`.
  ⚠️ **Дельта — из `pcp` (перевыверено вживую 2026-08-16, правило 07.06 было ошибочным).**
  `ClosePrice` врёт двумя способами: у валютных пар он **из 2023 года** (у `EUR/RUR` лежало 80.1
  при живых 97.70 → на экране был фейковый ▲+22 %), а у биржевых бумаг после закрытия сессии
  он **перезаписывается последней сделкой** → дневное движение схлопывается в 0.00 % (у `BRNT.EU`
  реальные −1.13 % показывались как 0). `chg` (абсолют) у валютных пар считается от того же
  протухшего close (+17.60 у `EUR/RUR`), поэтому берётся только когда сходится с `pcp`.
  Порядок в `freedom.quoteDelta`: `pcp` → `ltp−ClosePrice` → `chg`.
  Символы: рубль = **RUR** (`EUR/RUR` и т.п.), Brent-прокси = **`BRNT.EU`** (прямого бенчмарка нет;
  поиск тикеров — команда `tickerFinder`).
  Батч котировок устойчив к зависанию одного тикера: у каждого символа свой дедлайн
  (`perQuoteTimeout` 6 с), частичный результат отдаётся (лог `partial quote batch`), а
  «no quotes resolved» теперь несёт причину (`%w`).
- `getMarketReviews` (новости) → `list[]` с `title`/`date`. favqs + Open-Meteo — ок.

**Запуск:**
- `clock --ambient on|off` — переключить боковую подсветку прямо сейчас и выйти (`on` = случайный вид).
...
- [x] Боковая RGB-подсветка по расписанию (вкл 07:00 со случайным видом / выкл 22:30) — в сервисе,
      `AMBIENT_*` в ENV, проверено на устройстве и на сервере (2026-08-10).
